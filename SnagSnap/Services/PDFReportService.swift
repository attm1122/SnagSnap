...
class PDFReportService {
    ...
    /// - Throws: `PDFReportError` or file storage errors on failure.
    ...
    private func renderCoverPage(
        ...
        // Report type badge — NOT Optional (computed property)
        let reportTypeText = report.reportType.rawValue as NSString
        ...
        let typeSize = reportTypeText.size(withAttributes: typeAttributes)
        let typeX = typeBadgeRect.midX - typeSize.width / 2
        let typeY = typeBadgeRect.midY - typeSize.height / 2
        reportTypeText.draw(at: CGPoint(x: typeX, y: typeY), withAttributes: typeAttributes)
        ...
        // Property name — NOT Optional
        ("PROPERTY" as NSString).draw(
            at: CGPoint(x: margin, y: currentY),
            withAttributes: sectionAttributes
        )
        currentY += 18
        (report.propertyName as NSString).draw(
            at: CGPoint(x: margin, y: currentY),
            withAttributes: valueAttributes
        )
        currentY += 45

        // Property address — NOT Optional
        ("ADDRESS" as NSString).draw(
            at: CGPoint(x: margin, y: currentY),
            withAttributes: sectionAttributes
        )
        currentY += 18
        (report.propertyAddress as NSString).draw(
            at: CGPoint(x: margin, y: currentY),
            withAttributes: valueAttributes
        )
        currentY += 45

        // Inspector details — inspectorName IS Optional
        ...
        // Company details — companyName comes from settings ONLY (NOT from report)
        if settings.includeInspectorDetails, let companyName = settings.companyName, !companyName.isEmpty {
        ...
        // Contact info — companyPhone and companyEmail ARE Optional (from settings)
        if settings.includeInspectorDetails {
        ...
        // Date — createdAt is NOT Optional
        let dateFormatter = DateFormatter()
        dateFormatter.dateStyle = .long
        dateFormatter.timeStyle = .none
        let dateString = dateFormatter.string(from: report.createdAt)
    ...
    private func renderSummaryPage(
        ...
        // Report status — status is NOT Optional (computed property)
        ("STATUS" as NSString).draw(
            at: CGPoint(x: margin + 360, y: currentY + 12),
            withAttributes: overviewLabelAttributes
        )
        let statusText = report.status.rawValue as NSString
        statusText.draw(
        ...
        // Count issues by severity — severity is NOT Optional (computed property)
        var severityCounts: [IssueSeverity: Int] = [:]
        for issue in issues {
            let sev = issue.severity
            severityCounts[sev, default: 0] += 1
        }

        // Sort severities by importance: urgent > high > medium > low
        let severityOrder: [IssueSeverity] = [.urgent, .high, .medium, .low]

        for severity in severityOrder {
            guard let count = severityCounts[severity], count > 0 else { continue }
            let rowRect = CGRect(x: margin, y: currentY, width: contentWidth, height: 36)

            // Alternating row background
            let rowIndex = severityOrder.firstIndex(of: severity) ?? 0
            if rowIndex % 2 == 0 {
            ...
            let sevColor = self.severityColor(severity)
            let indicatorRect = CGRect(x: margin + 10, y: currentY + 10, width: 16, height: 16)
            cgContext.setFillColor(sevColor.cgColor)
            cgContext.fillRoundedRect(indicatorRect, cornerRadius: 4)

            // Severity name — rawValue is String (NOT Optional)
            let nameAttributes: [NSAttributedString.Key: Any] = [
                .font: UIFont.systemFont(ofSize: 14, weight: .medium),
                .foregroundColor: UIColor.darkText
            ]
            let severityDisplayName = severity.rawValue.capitalized as NSString
            severityDisplayName.draw(
        ...
        if !areas.isEmpty {
            ...
            for (index, area) in areas.enumerated() {
                // area.name is NOT Optional
                let areaIssueCount = area.issues?.count ?? 0
                let areaText = "\u{2022} \(area.name) (\(areaIssueCount) issues)" as NSString
    ...
    private func renderIssuePage(
        ...
        // Severity badge — severity is NOT Optional (computed property)
        let severity = issue.severity
        let severityText = severity.rawValue.capitalized as NSString
        ...
        // Status badge — status is NOT Optional (computed property)
        if settings.includeIssueStatuses {
            let status = issue.status
            let statusText = status.rawValue.capitalized.replacingOccurrences(of: "_", with: " ") as NSString
        ...
        // Notes section — notes IS Optional
    ...
    private func severityColor(_ severity: IssueSeverity) -> UIColor {
        switch severity {
        case .low:
            return UIColor(red: 0.18, green: 0.52, blue: 0.96, alpha: 1.0)      // Blue
        case .medium:
            return UIColor(red: 1.0, green: 0.65, blue: 0.15, alpha: 1.0)       // Orange/Yellow
        case .high:
            return UIColor(red: 0.95, green: 0.58, blue: 0.13, alpha: 1.0)      // Dark Orange
        case .urgent:
            return UIColor(red: 0.92, green: 0.26, blue: 0.21, alpha: 1.0)      // Red
        }
    }
    ...
    private func statusColor(_ status: IssueStatus) -> UIColor {
        ...
        case .notAnIssue:
            return UIColor.gray                                                  // Gray
        case .archived:
            return UIColor(red: 0.45, green: 0.45, blue: 0.45, alpha: 1.0)       // Dark gray
        }
    }
}
